...
class HistoricalPriorEngine:
    ...
    # ── 4. Hidden Markov Model — Viterbi form decode ─────────

    def _run_hmm_viterbi(self) -> np.ndarray:
        """
        2-state Gaussian HMM per team.
        ...
        for i in range(10):
            ...
            scores = rng.normal(loc=base_score, scale=15, size=(8, 1))
            ...
            _, state_seq = model.decode(scores, algorithm="viterbi")

            # Identify which state has the higher mean (= In-Form)
            means = model.means_.flatten()
            in_form_state = int(np.argmax(means))

            # Fraction of recent matches spent in In-Form state
            form_fraction = np.mean(state_seq == in_form_state)
